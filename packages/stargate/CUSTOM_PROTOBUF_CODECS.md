...
- You have installed `@ezstaking/proto-signing`, `@ezstaking/stargate` and
  `@ezstaking/tendermint-rpc` as dependencies. In general these dependencies should
  all have the same version, and this document is accurate as of version 0.26.
  ```
  "dependencies": {
    "@ezstaking/proto-signing": "^0.26.4",
    "@ezstaking/stargate": "^0.26.4",
    "@ezstaking/tendermint-rpc": "^0.26.4",
    // ...
  }
  ```
...
script we used for the `@ezstaking/stargate` package
...
`@ezstaking/proto-signing` for you to use, which maps type URLs to codec objects.
For example:

```ts
import { DirectSecp256k1HdWallet, Registry } from "@ezstaking/proto-signing";
import {
...
```ts
import { createProtobufRpcClient, QueryClient } from "@ezstaking/stargate";
import { Tendermint34Client } from "@ezstaking/tendermint-rpc";
...
Additionally, we provide a system for extending `@ezstaking/stargate`’s
